...
class QNetwork(nn.Module):
    def __init__(self, env: gym.Env) -> None:
        ...
        self.advantage_stream = nn.Linear(84, env.action_space.n)

    def forward(self, observation) -> Tensor:
        features = self.feauture_layer(observation)
...
# Network setup
q_network1 = QNetwork(env)
q_network2 = QNetwork(env)
optimizer = optim.Adam(q_network1.parameters(), lr=learning_rate)
target_network = QNetwork(env)
target_network.load_state_dict(q_network1.state_dict())
...
while global_step < total_timesteps:
    ...
    if np.random.random() < epsilon:
        ...
        q_values = q_network1(observation)
    ...
    if global_step >= learning_starts:
        if global_step % train_frequency == 0:
            ...
            old_val = q_network1(b_observations)[range(batch_size), b_actions]
